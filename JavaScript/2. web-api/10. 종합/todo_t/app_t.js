...
function makeNewToDoNode(newTodo) {
  ...
  // 클래스 이름을 두개 add할떄는 매개값으리 두개 보내야함.
  // 한번에 공백 포함 두개 설정하시면 에러입니다.
...
function changeCheckState($label) {
  ...
  // checked를 상황에 따라서 추가하거나 삭제할 수 있어야 하기 떄문에
  // toggle함수로 처리해 주겠습니다.
...
function removeToDoData($delTarget) {
  ...
  // removeChild 진행 전에 애미메이션 발동 및 배열 내부에 객체 삭제가 진행될 수 있도록 시간을 지연
  setTimeout(() => {
